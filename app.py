...
def generate_pdf_report(tsb, risk, b_val, a_val, p_name, p_age, p_gender, img_target_array=None, img_crop_array=None):
    class PDF(FPDF):
        def header(self):
            # Clinical Teal Header Band
            self.set_fill_color(31, 138, 120)
            self.rect(0, 0, 210, 35, 'F')
            self.set_y(12)
            self.set_font("Helvetica", 'B', 20)
            self.set_text_color(255, 255, 255)
            self.cell(0, 10, "COLORCAL-JAUNDICE PRO", new_x="LMARGIN", new_y="NEXT", align="C")
            self.set_font("Helvetica", '', 11)
            self.cell(0, 6, "OFFICIAL CLINICAL TRIAGE REPORT", new_x="LMARGIN", new_y="NEXT", align="C")
            self.set_y(45)
            
        def footer(self):
            # Big Red Medical Disclaimer (Bottom of page)
            self.set_y(-30)
            ...
            self.cell(0, 6, "MEDICAL DISCLAIMER & LIABILITY", new_x="LMARGIN", new_y="NEXT", align="C")
            
            self.set_font("Helvetica", 'B', 7)
            self.set_text_color(50, 50, 50)
            disclaimer = "This application is a student prototype for non-invasive screening. It is NOT an FDA-approved diagnostic device. It absolutely DOES NOT replace standard serological blood tests (Serum Bilirubin / LFTs). Do not use this tool for final medical diagnosis."
            self.set_x(12)
            self.multi_cell(186, 4, disclaimer, align="C")

    pdf = PDF()
    pdf.add_page()
    
    display_name = p_name.strip() if p_name.strip() else "ANONYMOUS"
    age_str = str(p_age) if p_age > 0 else "N/A"
    gender_str = p_gender if p_gender != "Unspecified" else "N/A"
    date_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    # Patient Details Box (Shaded Gray)
    ...
    pdf.set_xy(15, 48)
    pdf.cell(40, 6, "PATIENT NAME / ID:")
    pdf.set_text_color(0, 0, 0)
    pdf.cell(60, 6, display_name)
    
    pdf.set_text_color(100, 100, 100)
    pdf.cell(20, 6, "DATE:")
    pdf.set_text_color(0, 0, 0)
    pdf.cell(50, 6, date_str)
    
    pdf.set_xy(15, 56)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(20, 6, "AGE:")
    pdf.set_text_color(0, 0, 0)
    pdf.cell(30, 6, age_str)
    
    pdf.set_text_color(100, 100, 100)
    pdf.cell(25, 6, "GENDER:")
    pdf.set_text_color(0, 0, 0)
    pdf.cell(30, 6, gender_str)
    
    pdf.set_y(78)
    
    # Results Section
    pdf.set_font("Helvetica", 'B', 14)
    pdf.set_text_color(31, 138, 120)
    pdf.cell(0, 10, "CLINICAL FINDINGS", new_x="LMARGIN", new_y="NEXT", border="B")
    pdf.ln(4)
    
    # Big TSB Result
    pdf.set_font("Helvetica", '', 12)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(85, 10, "Estimated Serum Bilirubin (TSB):")
    pdf.set_font("Helvetica", 'B', 16)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 10, f"~ {tsb} mg/dL", new_x="LMARGIN", new_y="NEXT")
    
    # Risk Level (Dynamic Coloring)
    if "LOW" in risk.upper():
        r, g, b = 34, 197, 94 # Green
    elif "ELEVATED" in risk.upper():
        r, g, b = 245, 158, 11 # Amber
    elif "MODERATE" in risk.upper():
    ...
    pdf.set_font("Helvetica", '', 12)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(85, 10, "Triage Risk Level:")
    pdf.set_font("Helvetica", 'B', 14)
    pdf.set_text_color(r, g, b)
    pdf.cell(0, 10, risk.upper(), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(10)
    
    # Biomarker Data Table
    pdf.set_font("Helvetica", 'B', 14)
    pdf.set_text_color(31, 138, 120)
    pdf.cell(0, 10, "BIOMARKER METRICS", new_x="LMARGIN", new_y="NEXT", border="B")
    pdf.ln(4)
    
    pdf.set_fill_color(248, 250, 252)
    pdf.set_draw_color(220, 220, 220)
    
    def add_row(col1, col2, fill=False):
        pdf.set_font("Helvetica", '', 11)
        pdf.set_text_color(80, 80, 80)
        pdf.cell(95, 10, f"  {col1}", border=1, fill=fill)
        pdf.set_font("Helvetica", 'B', 11)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(95, 10, f"  {col2}", border=1, fill=fill, new_x="LMARGIN", new_y="NEXT")
        
    add_row("Analyzed Target Area", "Sclera (AI Isolated)", True)
    add_row("Pure Yellowness (b* CIELAB)", f"{b_val:.1f}", False)
    add_row("Vascular Correction (a* CIELAB)", f"{a_val:.1f}", True)
    pdf.ln(10)
    
    # --- VISUAL CLINICAL EVIDENCE ---
    if img_target_array is not None and img_crop_array is not None:
        pdf.set_font("Helvetica", 'B', 14)
        pdf.set_text_color(31, 138, 120)
        pdf.cell(0, 10, "VISUAL CLINICAL EVIDENCE", new_x="LMARGIN", new_y="NEXT", border="B")
        pdf.ln(4)
        
        # Helper to convert OpenCV BGR/RGB to BytesIO
        def ndarray_to_bytesio(img_array):
            import io
            from PIL import Image
            # Streamlit arrays are already RGB usually, but if OpenCV it's BGR. 
            # We assume it's RGB since we use it directly in st.image()
        ...
        img_target_io = ndarray_to_bytesio(img_target_array)
        img_crop_io = ndarray_to_bytesio(img_crop_array)
        
        y_before = pdf.get_y()
        # The target image is portrait. We'll constrain height to 50mm.
        pdf.image(img_target_io, x=30, y=y_before, h=50)
        # The crop image is landscape. We'll constrain height to 20mm and center it alongside.
        pdf.image(img_crop_io, x=120, y=y_before + 15, h=20)
...
st.set_page_config(page_title="ColorCal-Jaundice Pro", page_icon="🩺", layout="wide", initial_sidebar_state="expanded")

# --- PATIENT DEMOGRAPHICS (SIDEBAR) ---
with st.sidebar:
    st.markdown("### 📋 Patient Demographics")
    st.markdown("<p style='font-size:0.85rem; color:#4B5A65;'>*(Optional for privacy)*</p>", unsafe_allow_html=True)
    patient_name = st.text_input("Patient Name / ID", value="", placeholder="e.g., John Doe or ID-492")
...
with c2: 
    ...
    if uploaded_file is not None:
        ...
        if use_calibration:
            ...
            # --- FEATURE 1: LIGHTING CONFIDENCE FAIL-SAFE ---
            mean_card_luma = np.mean(captured_colors)
            if mean_card_luma < 60:
                st.warning("⚠️ **POOR LIGHTING (Shadows):** The reference card is heavily shadowed. The Polynomial AI will attempt to reconstruct the colors, but clinical accuracy may be reduced. Please ensure the card is evenly lit.")
            elif mean_card_luma > 200:
                st.warning("⚠️ **POOR LIGHTING (Glare):** The reference card is washed out by a camera flash or harsh glare. The AI will attempt to correct this, but clinical accuracy may be reduced.")
            else:
                st.success("✅ **Lighting Confidence: HIGH.** Card exposure is optimal for clinical calibration.")
            # ------------------------------------------------
        ...
        with col_sc1:
            ...
            st.image(overlay, caption=f"Target Area: {eye_label}", use_container_width=True)
        ...
        with col_sc2:
            ...
            # Clinical Sclera Masking: Pure Luminance Top-Percentile
            # Because our rectangular crop box perfectly dodges the eyelids, the box contains only: Pupil/Iris (Dark) and Sclera (Bright).
            # By isolating the absolute brightest 30% of pixels (and excluding specular glare >245), 
            # we mathematically guarantee we isolate the sclera, without accidentally deleting highly-saturated jaundiced pigment!
            L_thresh = np.percentile(L_chan, 70)
            valid_mask = (L_chan >= L_thresh) & (L_chan < 245)
            if not np.any(valid_mask):
                valid_mask = (L_chan >= np.percentile(L_chan, 50)) & (L_chan < 250)
            ...
            with col_roi1:
                st.image(sc_roi, caption="AI Cropped Eye", use_container_width=True)
            with col_roi2:
                st.image(mask_visual, caption="Skin & Glare Rejected", use_container_width=True)
            ...
            # CLINICAL BASELINE CORRECTION:
            # A healthy human sclera is not mathematically pure white (b*=0); 
            # it naturally rests between b*=1.0 and b*=3.5 due to natural tissue density.
            # We subtract a physiological baseline of 3.0 before calculating pathological TSB.
            excess_b = max(0, b_rounded - 3.0)
            
            # ADULT/GENERAL POPULATION TSB MATH:
            # Healthy adult TSB is 0.1 to 1.2. Scleral Icterus becomes visible at TSB > 2.5.
            est_tsb = max(0.2, round((excess_b * 0.6) + 0.5, 1))
            ...
            <div style="border:1px solid {risk_border}; border-radius:8px; overflow:hidden; background:transparent; font-family:'IBM Plex Sans', sans-serif;">
                <div style="background:{risk_bg}; padding:16px 20px; border-bottom:1px solid {risk_border};">
                    <div style="color:{risk_color}; font-family:'IBM Plex Mono', monospace; font-size:0.75rem; font-weight:700; letter-spacing:0.05em; margin-bottom:4px;">{risk_level} (Sclera b* = {b_rounded:.1f})</div>
                ...
                <div style="padding:0;">
                    <table style="width:100%; border-collapse:collapse; font-size:0.9rem; text-align:left;">
                        ...
                        <tr style="border-bottom:1px solid var(--line); background-color:#F8FAFC;">
                            ...
                            <td style="padding:12px 20px; font-weight:700; color:#14202E; font-size:1.05rem;">~ {est_tsb:.1f} mg/dL</td>
                        </tr>
                        <tr style="border-bottom:1px solid var(--line);">
                            <td style="padding:12px 20px; color:var(--ink-soft);">Pure Yellowness (b*)</td>
                            <td style="padding:12px 20px; font-family:'IBM Plex Mono', monospace; color:var(--ink);">{b_rounded:.1f}</td>
                        </tr>
                        <tr style="border-bottom:1px solid var(--line);">
                            <td style="padding:12px 20px; color:var(--ink-soft);">Vascular Correction (a*)</td>
                            <td style="padding:12px 20px; font-family:'IBM Plex Mono', monospace; color:var(--ink);">{a_rounded:.1f}</td>
                        </tr>
                        <tr>
            ...
            pdf_bytes = generate_pdf_report(
                ...
                patient_age, 
                patient_gender,
                overlay,  # Pass the target area image
                sc_roi    # Pass the AI cropped eye image
            )
            st.download_button(
                label="📄 Download Medical Triage Report (PDF)",
                data=pdf_bytes,
